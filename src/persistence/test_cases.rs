...
use crate::persistence::assertions::AssertionsTable;
use crate::persistence::parameters::ParametersTable;
...
impl TestCaseOperations {
    ...
    pub async fn delete(&self, customer_id: &String, test_case_id: &String) {
        ...
        let deleted_test_case = TestCaseTable::delete_item(self.client.clone(), customer_id.clone(), test_case_id.clone()).await;
        if let Ok(Some(deleted_case)) = deleted_test_case {
            tx.send(crate::persistence::repo::OnDeleteMessage::TestCaseDeleted(deleted_case)).await.unwrap();
        }
        let tx3 = tx.clone();
        ...
        tokio::task::spawn(async move {
            while let Some(message) = rx.recv().await {
                match message {
                    crate::persistence::repo::OnDeleteMessage::TestCaseDeleted(test_case) => {
                        ActionsTable::delete_all_items(cloned_client.clone(), build_composite_key(vec![test_case.customer_id.clone(), test_case.id.clone()]), None, &tx3).await;
                    }
                    crate::persistence::repo::OnDeleteMessage::ActionDeleted(action) => {
                        ParametersTable::delete_all_items(cloned_client.clone(), build_composite_key(vec![action.customer_id.clone(), action.test_case_id.clone()]), None, &tx3).await;
                        AssertionsTable::delete_all_items(cloned_client.clone(), build_composite_key(vec![action.customer_id.clone(), action.test_case_id.clone()]), None, &tx3).await;
                    }
                }
            }
        });
